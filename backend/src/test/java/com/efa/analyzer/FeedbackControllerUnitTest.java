...
import com.efa.analyzer.backend.controller.FeedbackController;
...
class FeedbackControllerUnitTest {
  ...
  void setUp() {
    ...
    mvc =
        MockMvcBuilders.standaloneSetup(
                new FeedbackController(feedbackCommandService, feedbackQueryService))
  ...
  @Test
  @DisplayName(
      "POST /api/events/{eventId}/feedback -> 201 returns FeedbackResponse JSON and calls service with DTO")
  void post_feedback_create_201_returnsJson() throws Exception {
    ...
    when(feedbackCommandService.create(eq(42), any(FeedbackCreateRequest.class)))
        .thenReturn(
            new FeedbackResponse(555, 42, "Amazing workshop!", Sentiment.POSITIVE, createdAt));
    ...
    mvc.perform(
            post("/api/events/{eventId}/feedback", 42)
    ...
    ArgumentCaptor<FeedbackCreateRequest> captor =
        ArgumentCaptor.forClass(FeedbackCreateRequest.class);
  ...
  void post_feedback_400_whenBlankContent() throws Exception {
    ...
    mvc.perform(
            post("/api/events/{eventId}/feedback", 7)
                .contentType(MediaType.APPLICATION_JSON)
                .content(badBody))
  ...
  void get_feedback_list_for_event_200() throws Exception {
    ...
    when(feedbackQueryService.getAll(9))
        .thenReturn(
            List.of(
  ...
  @Test
  @DisplayName(
      "GET /api/events/{eventId}/feedback/{feedbackId} -> 200 returns single feedback item")
